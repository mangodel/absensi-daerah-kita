...
export default function EventList({ events, sessions = [], onEdit, onDelete, onSelectForAttendance }) {
  ...
  return (
    <div className="space-y-3">
      {events.map(event => {
        const linkedSession = sessions.find(s => s.linked_event_id === event.id);
        return (
            ...
            <div className="space-y-2 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {linkedSession && (
                  <Badge
                    variant="outline"
                    className="text-xs bg-primary/10 text-primary border-primary/30 flex items-center gap-1 cursor-pointer hover:bg-primary/20"
                    onClick={() => navigate(`/event-attendance`)}
                  >
                    <QrCode className="w-2.5 h-2.5" />
                    QR {linkedSession.status}
                  </Badge>
                )}
              </div>
